...
GPU_ID="${GPU_ID:-1}"
EXP="${EXP:-errnet_phaseB_full_improved_ctx}"
...
conda run --no-capture-output -n "${ENV_NAME}" \
  ...
  python train_errnet_unaligned.py \
    ...
    --no-log \
    --no_html \
  2>&1 | tee "experiments/run-logs/phaseB_full_train_improved_ctx_${RUN_ID}.log"
status=${PIPESTATUS[0]}
set -e

echo "${status}" > "experiments/run-logs/phaseB_full_train_improved_ctx_${RUN_ID}.status"
exit "${status}"
